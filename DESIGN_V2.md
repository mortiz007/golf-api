...
Base: `/api`. Las rutas autenticadas aplican `auth:sanctum` y `throttle:60,1`.
...
Header `Location: /api/listings/{id}`. Cuerpo plano (sin envoltura `data`). `ai_enrichment` siempre presente (null en creación).

**Códigos:** `201`, `422`, `401`, `429`.

### 3.3 `PATCH /api/listings/{id}` — Actualizar parcial (protegido, solo propietario)
...
### 3.4 `DELETE /api/listings/{id}` — Cancelar (protegido, solo propietario)
...
### 3.5 `GET /api/listings` — Listado público (sin auth)
...
### 3.6 `GET /api/audit-logs` — Auditoría (protegido)
...
`GET /api/listings` usa un lado de lectura separado del de escritura:
...
- **Ruta pública:** `GET /api/listings` se registra fuera del grupo `auth:sanctum`/`throttle`.
